document.addEventListener("DOMContentLoaded", function() {
    ...
    function satirliGuvenliMetin(deger) {
        return guvenliMetin(deger).replace(/\n/g, "<br>");
    }

    // --- 1. KISIM: Sidebar (Açılır Kapanır) Menü Mantığı ---
    const weekHeaders = Array.from(document.querySelectorAll('.accordion-header'));
    const dayHeaders = Array.from(document.querySelectorAll('.day-header'));

    function okuAyarla(header, acikMi) {
        const arrow = header ? header.querySelector('.arrow') : null;
        if (arrow) arrow.textContent = acikMi ? "▲" : "▼";
    }

    function panelAyarla(header, acikMi) {
        const content = header ? header.nextElementSibling : null;
        if (!content) return;

        content.style.display = acikMi ? "block" : "none";
        okuAyarla(header, acikMi);
    }

    function panelAcikMi(header) {
        const content = header ? header.nextElementSibling : null;
        return content ? content.style.display === "block" : false;
    }

    function tumGunleriKapat(haricGunHeader = null) {
        dayHeaders.forEach(header => {
            if (header !== haricGunHeader) panelAyarla(header, false);
        });
    }

    function tumHaftalariKapat(haricHaftaHeader = null) {
        weekHeaders.forEach(header => {
            if (header !== haricHaftaHeader) panelAyarla(header, false);
        });
    }

    function gununHaftaHeaderiniBul(dayHeader) {
        let parent = dayHeader ? dayHeader.parentElement : null;

        while (parent && parent !== document.body) {
            const oncekiEleman = parent.previousElementSibling;
            if (oncekiEleman && oncekiEleman.classList && oncekiEleman.classList.contains('accordion-header')) {
                return oncekiEleman;
            }
    ...
    function butonunGunHeaderiniBul(buton) {
        const liste = buton ? buton.closest('.exercise-list') : null;
        const oncekiEleman = liste ? liste.previousElementSibling : null;
        return oncekiEleman && oncekiEleman.classList.contains('day-header') ? oncekiEleman : null;
    }

    // Sayfa ilk açıldığında sadece hafta başlıkları görünsün.
    tumGunleriKapat();
    tumHaftalariKapat();

    weekHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const acikti = panelAcikMi(this);

            tumHaftalariKapat(this);
            tumGunleriKapat();
            panelAyarla(this, !acikti);
        });
    });
    ...
    dayHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const acikti = panelAcikMi(this);
            const haftaHeader = gununHaftaHeaderiniBul(this);

            if (haftaHeader) {
                tumHaftalariKapat(haftaHeader);
                panelAyarla(haftaHeader, true);
            }

            tumGunleriKapat(this);
            panelAyarla(this, !acikti);
        });
    });
    ...
    function ekranaYazdir(veri, baslik, tur, secilenGun) {
        const oncekiTooltip = document.getElementById('ceviri-tooltip');
        if (oncekiTooltip) oncekiTooltip.style.display = 'none';

        if (tur !== "kelime_alistirmasi" && (!veri || (Array.isArray(veri) && veri.length === 0) || (typeof veri === "object" && Object.keys(veri).length === 0))) {
            icerikAlani.innerHTML = `
        ...
        if (tur === "okuma_alistirmasi") {
            ...
            let htmlIcerik = `
                <div class="content-header">
                    <p>${guvenliMetin(baslik)}</p>
                </div>
                <div class="okuma-kapsayici" style="max-width: 800px; margin: 0 auto;">
                    ${veri.resim ? `<img src="${guvenliMetin(veri.resim)}" alt="Okuma Görseli" style="width: 100%; border-radius: 10px; margin-bottom: 20px; object-fit: cover; max-height: 400px;">` : ''}
                    ...
                    <div style="position: relative;">
                        <button id="translate-btn" style="position: absolute; top: 10px; right: 10px; background: #f8f9fa; border: 1px solid #ddd; border-radius: 5px; padding: 5px 15px; cursor: pointer; font-size: 13px; font-weight: 600; color: #333; z-index: 10; transition: all 0.3s ease;">
                            <span id="btn-text">TR</span>
                        </button>

                        <div class="okuma-metni" id="okumaMetniAlani" style="font-size: 24px; line-height: 1.8; text-align: right; margin-bottom: 30px; padding: 45px 20px 20px 20px; background-color: #f9f9f9; border-radius: 10px; transition: all 0.3s ease;" dir="rtl"></div>
                    </div>
                    ...
                    veri.onemli_ifadeler[seviye].forEach(ifade => {
                        kelimeHtml += `
                            <div class="kelime-card" style="border-left: 4px solid #1a5c83;">
                                <h4 dir="rtl" style="font-size: 20px; margin: 0 0 10px 0;">${guvenliMetin(ifade.arapca)}</h4>
                                <p style="margin: 0; color: #555;">${guvenliMetin(ifade.turkce)}</p>
                            </div>
                        `;
                    });
                    kelimeHtml += `</div>`;
                    ...
                    veri.zor_kelimeler[seviye].forEach(kelime => {
                        kelimeHtml += `
                            <div class="kelime-card">
                                <h4 dir="rtl" style="font-size: 20px; margin: 0 0 10px 0; color: #1a5c83;">${guvenliMetin(kelime.arapca)}</h4>
                                <p style="margin: 0 0 5px 0;"><strong>Çeviri:</strong> ${guvenliMetin(kelime.turkce)}</p>
                                ${kelime.anlam ? `<p style="margin: 0 0 5px 0; font-size: 14px; color: #666;" dir="rtl">${guvenliMetin(kelime.anlam)}</p>` : ''}
                                <p style="margin: 0; font-style: italic; color: #888; font-size: 14px;" dir="rtl"><strong>Örnek:</strong> ${guvenliMetin(kelime.ornek)}</p>
                            </div>
                        `;
                    });
                    kelimeHtml += `</div>`;
            ...
            function metniTiklanabilirYap(metin) {
                if (!metin) return "";

                return String(metin).split(/(\s+)/).map(parca => {
                    if (parca.trim() === "") return parca;
                    return `<span class="okuma-kelimesi" style="cursor: pointer; display: inline-block; transition: 0.2s; border-radius: 4px; padding: 0 2px;">${guvenliMetin(parca)}</span>`;
                }).join('');
            }
            ...
            metinAlani.addEventListener('click', function(e) {
                ...
                const tiklananSafKelime = span.innerText;
                const zorKelimeler = veri.zor_kelimeler && veri.zor_kelimeler[aktifSeviye] ? veri.zor_kelimeler[aktifSeviye] : [];
                const gizliSozluk = veri.okuma_sozlugu || [];
                const tumKelimeler = [...zorKelimeler, ...gizliSozluk];
                const bulunanKelime = tumKelimeler.find(k => kelimeEslesiyorMu(tiklananSafKelime, k.arapca));
                ...
                if (bulunanKelime) {
                    tooltip.innerHTML = `
                        <div style="text-align:center;">
                            <div dir="rtl" style="color:#ffd700; font-size:22px; font-weight:700; margin-bottom:2px;">${guvenliMetin(bulunanKelime.arapca)}</div>
                            <div style="color:white; font-size:16px; font-weight:500;">${guvenliMetin(bulunanKelime.turkce)}</div>
                        </div>
                    `;
                } else {
                    tooltip.innerHTML = `<span style="font-size:14px; color:#e0e0e0;">Sözlükte yok (${guvenliMetin(arapcaTemizle(tiklananSafKelime))})</span>`;
                }
            ...
            if (window.okumaDokumanTiklamaDinleyicisi) {
                document.removeEventListener('click', window.okumaDokumanTiklamaDinleyicisi);
            }

            window.okumaDokumanTiklamaDinleyicisi = function(e) {
            ...
            document.addEventListener('click', window.okumaDokumanTiklamaDinleyicisi);

            metinAlani.innerHTML = metniTiklanabilirYap(metinler.basit);
            ...
            translateBtn.addEventListener('click', function() {
                if (!isTurkish) {
                    metinAlani.innerHTML = satirliGuvenliMetin(metinler[aktifSeviye + "_tr"] || "Bu metnin Türkçe çevirisi henüz eklenmemiş.");
                    metinAlani.style.direction = "ltr";
                    metinAlani.style.textAlign = "left";
                    metinAlani.style.fontSize = "16px";
                    btnText.innerText = "AR";
                    isTurkish = true;
                    tooltip.style.display = 'none';
                } else {
                    metinAlani.innerHTML = metniTiklanabilirYap(metinler[aktifSeviye]);
                    metinAlani.style.direction = "rtl";
            ...
            seviyeButonlari.forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    // Seviye Değişince Çeviriyi Arapçaya Sıfırla
                    isTurkish = false;
                    const btnText = document.getElementById('btn-text');
                    if(btnText) btnText.innerText = "TR";
                    metinAlani.style.direction = "rtl";
                    metinAlani.style.textAlign = "right";
                    metinAlani.style.fontSize = "24px";
                    ...
                    // --- YENİ EKLENEN KISIM: SES DOSYASINI GÜNCELLEME ---
                    const sesOynatici = document.getElementById('okuma-ses-oynatici');
                    const sesKaynagi = document.getElementById('okuma-ses-kaynagi');
                    
                    if (sesOynatici && sesKaynagi) {
                        // Seçilen seviyeye ait ses dosyasını bul
                        let yeniSes = veri.sesler && veri.sesler[aktifSeviye] ? veri.sesler[aktifSeviye] : (veri.ses || '');
                        
                        // Sesi değiştir ve oynatıcıyı yeniden yükle
                        sesKaynagi.src = yeniSes;
                        sesOynatici.load(); 
                    }
                });
            });
        ...
        // =========================================================================
            ...
            const kelimeler = Array.isArray(veri)
                ? veri
                : ((typeof egitimMufredati !== 'undefined' && egitimMufredati[secilenGun]) ? egitimMufredati[secilenGun]["kelime_alistirmasi"] : null);
            ...
            function metniTemizle(metin) {
                return String(metin ?? "").trim().toLocaleLowerCase('tr-TR').replace(/[.,?!]/g, '');
            }

            function arapcaSeslendir(metin) {
                if (!('speechSynthesis' in window)) return;

                window.speechSynthesis.cancel();
                const utterance = new SpeechSynthesisUtterance(String(metin || ""));
            ...
            const bilinenKartlar = new Set();

            function kartlarModunuCiz() {
                const mevcutKelime = kelimeler[aktifKartIndeksi];
                const ogrenildiMi = bilinenKartlar.has(aktifKartIndeksi);
                let noktalarHtml = '';

                for (let i = 0; i < kelimeler.length; i++) {
                    if (i === aktifKartIndeksi) {
                        noktalarHtml += `<span class="dot active" style="height: 10px; width: 10px; background-color: #1a5c83; border-radius: 50%; display: inline-block; margin: 0 4px;"></span>`;
                    } else if (bilinenKartlar.has(i)) {
                        noktalarHtml += `<span class="dot passed" style="height: 10px; width: 10px; background-color: #28a745; border-radius: 50%; display: inline-block; margin: 0 4px;"></span>`;
                    } else {
                        noktalarHtml += `<span class="dot" style="height: 10px; width: 10px; background-color: #ddd; border-radius: 50%; display: inline-block; margin: 0 4px;"></span>`;
                    }
                }
                    ...
                    <div class="flashcard-container" style="display: flex; flex-direction: column; align-items: center; perspective: 1000px;">
                        <div class="flashcard" id="flashcard-flip" style="width: 100%; max-width: 550px; height: 320px; border: 1px solid #e0e0e0; border-radius: 12px; position: relative; cursor: pointer; transition: transform 0.6s; transform-style: preserve-3d; background-color: transparent;">
                            <div class="front" style="position: absolute; width: 100%; height: 100%; backface-visibility: hidden; display: flex; flex-direction: column; align-items: center; justify-content: center; background-color: white; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                                <span class="status-badge" style="position: absolute; top: 15px; right: 15px; background-color: ${ogrenildiMi ? '#eaffea' : '#f8f9fa'}; color: ${ogrenildiMi ? '#28a745' : '#666'}; border: 1px solid ${ogrenildiMi ? '#28a745' : '#ddd'}; padding: 5px 12px; border-radius: 20px; font-size: 13px; font-weight: 600;">${ogrenildiMi ? '✓ Öğrenildi' : 'Çalışılıyor'}</span>
                                <h1 class="word" dir="rtl" style="font-size: 42px; color: #333; margin: 0; display: flex; align-items: center; gap: 15px;">
                                    ${guvenliMetin(mevcutKelime.arapca)}
                            ...
                            <div class="back" style="position: absolute; width: 100%; height: 100%; backface-visibility: hidden; display: flex; flex-direction: column; align-items: center; justify-content: center; background-color: white; border-radius: 12px; transform: rotateX(180deg); box-shadow: 0 4px 10px rgba(0,0,0,0.05); border: 2px solid #1a5c83;">
                                <h1 class="word" style="font-size: 32px; color: #1a5c83; margin: 0; text-align: center; padding: 20px;">${guvenliMetin(mevcutKelime.turkce)}</h1>
                            </div>
                        </div>
                ...
                const audioIcon = flashcard.querySelector('.audio-icon');
                audioIcon.addEventListener('click', function(e) {
                    e.stopPropagation();
                    arapcaSeslendir(mevcutKelime.arapca);
                });

                document.getElementById('btn-sonraki').addEventListener('click', () => {
                    if (aktifKartIndeksi < kelimeler.length - 1) {
                        aktifKartIndeksi++;
                        kartlarModunuCiz();
                    }
                });
                ...
                document.getElementById('btn-biliyorum').addEventListener('click', () => {
                    bilinenKartlar.add(aktifKartIndeksi);
                    if (aktifKartIndeksi < kelimeler.length - 1) {
                        aktifKartIndeksi++;
                    }
                    kartlarModunuCiz();
            ...
            function ogrenmeModunuCiz() {
                if (ogrenmeIndeksi >= kelimeler.length) {
                    alistirmaIcerikAlani.innerHTML = `
                        <div style="text-align:center; padding:50px;">
                            <h2>Tebrikler! 🎉 Skor: ${ogrenmeSkoru}</h2>
                            <button id="btn-ogrenme-tekrar" class="btn btn-primary" style="margin-top:20px; padding:10px 20px; border:none; background:#1a5c83; color:white; border-radius:8px; cursor:pointer;">Tekrar Çöz</button>
                        </div>
                    `;
                    document.getElementById('btn-ogrenme-tekrar').onclick = () => {
                        ogrenmeIndeksi = 0;
                        ogrenmeSkoru = 0;
                        ogrenmeModunuCiz();
                    };
                ...
                alistirmaIcerikAlani.innerHTML = `
                    <div style="display: flex; flex-direction: column; align-items: center; margin-top: 30px;">
                        <div style="width: 100%; max-width: 550px; border: 1px solid #e0e0e0; border-radius: 12px; padding: 40px 20px; text-align: center; box-shadow: 0 4px 10px rgba(0,0,0,0.05); background: white;">
                            ...
                            <h2 style="font-size: 42px; margin-bottom:30px;" dir="rtl">${guvenliMetin(mevcutKelime.arapca)}</h2>
                ...
                input.focus();
                input.onkeypress = (e) => {
                    if (e.key === 'Enter') btnK.style.display === 'none' ? btnS.click() : btnK.click();
                };
                ...
                btnK.onclick = () => {
                    ...
                    if (metniTemizle(input.value) === metniTemizle(mevcutKelime.turkce)) {
                        ...
                        msg.innerHTML = `❌ Yanlış! Doğru: ${guvenliMetin(mevcutKelime.turkce)}`;
            ...
            function coktanSecmeliModunuCiz() {
                if (coktanIndeksi >= kelimeler.length) {
                    alistirmaIcerikAlani.innerHTML = `
                        <div style="text-align:center; padding:50px;">
                            <h2>Tebrikler! 🎉 Skor: ${coktanSkor}</h2>
                            <button id="btn-coktan-tekrar" style="margin-top:20px; padding:10px 20px; border:none; background:#1a5c83; color:white; border-radius:8px; cursor:pointer;">Tekrar Çöz</button>
                        </div>
                    `;
                    document.getElementById('btn-coktan-tekrar').onclick = () => {
                        coktanIndeksi = 0;
                        coktanSkor = 0;
                ...
                const siklar = [mevcut.turkce];
                const digerleri = kelimeler.filter(k => k.turkce !== mevcut.turkce).sort(() => 0.5 - Math.random());

                for (let i = 0; i < 3 && i < digerleri.length; i++) {
                    siklar.push(digerleri[i].turkce);
                }

                siklar.sort(() => 0.5 - Math.random());

                let sikHtml = '';
                siklar.forEach((s, index) => {
                    sikHtml += `<button class="cs-btn" data-index="${index}" style="width:46%; padding:15px; margin:2%; border:2px solid #e0e0e0; background:white; border-radius:8px; cursor:pointer; font-weight:500;">${guvenliMetin(s)}</button>`;
                });
                ...
                alistirmaIcerikAlani.innerHTML = `
                    <div style="display: flex; flex-direction: column; align-items: center; margin-top: 30px;">
                        <div style="width: 100%; max-width: 600px; border: 1px solid #e0e0e0; border-radius: 12px; padding: 40px 20px; text-align: center; box-shadow: 0 4px 10px rgba(0,0,0,0.05); background: white;">
                            <h2 style="font-size: 42px; margin-bottom:30px;" dir="rtl">${guvenliMetin(mevcut.arapca)}</h2>
                ...
                document.querySelectorAll('.cs-btn').forEach(b => {
                    b.onclick = function() {
                        ...
                        const secilenCevap = siklar[Number(this.getAttribute('data-index'))];
                        if (secilenCevap === mevcut.turkce) {
                            ...
                            this.style.color = "white";
                            document.querySelectorAll('.cs-btn').forEach(b2 => {
                                const b2Cevap = siklar[Number(b2.getAttribute('data-index'))];
                                if (b2Cevap === mevcut.turkce) {
                ...
                document.getElementById('btn-coktan-sonraki').onclick = () => {
                    coktanIndeksi++;
                    coktanSecmeliModunuCiz();
                };
            }
            ...
            function dogruYanlisModunuCiz() {
                if (dyIndeksi >= kelimeler.length) {
                    alistirmaIcerikAlani.innerHTML = `
                        <div style="text-align:center; padding:50px;">
                            <h2>Tebrikler! 🎉 Skor: ${dySkor}</h2>
                            <button id="btn-dy-tekrar" style="margin-top:20px; padding:10px 20px; border:none; background:#1a5c83; color:white; border-radius:8px; cursor:pointer;">Tekrar Çöz</button>
                        </div>
                    `;
                    document.getElementById('btn-dy-tekrar').onclick = () => {
                        dyIndeksi = 0;
                        dySkor = 0;
                        dogruYanlisModunuCiz();
                    };
                    return;
                }

                const mevcut = kelimeler[dyIndeksi];
                const yanlisSecenekler = kelimeler.filter(k => k.turkce !== mevcut.turkce);
                const dogruMu = yanlisSecenekler.length === 0 || Math.random() > 0.5;
                const gosterilenTurkce = dogruMu
                    ? mevcut.turkce
                    : yanlisSecenekler[Math.floor(Math.random() * yanlisSecenekler.length)].turkce;
                ...
                alistirmaIcerikAlani.innerHTML = `
                    <div style="display: flex; flex-direction: column; align-items: center; margin-top: 30px;">
                        <div style="width: 100%; max-width: 550px; border: 1px solid #e0e0e0; border-radius: 12px; padding: 40px 20px; text-align: center; box-shadow: 0 4px 10px rgba(0,0,0,0.05); background: white;">
                            ...
                            <h2 style="font-size: 42px; margin-bottom:10px;" dir="rtl">${guvenliMetin(mevcut.arapca)}</h2>
                            <h1 style="color:#1a5c83; margin-bottom:30px;">${guvenliMetin(gosterilenTurkce)}</h1>
                ...
                const bitir = (secim) => {
                    ...
                    if (secim === gercektenDogru) {
                        msg.innerHTML = "✅ Tebrikler, Doğru!";
                        msg.style.color = "#28a745";
                        dySkor += 10;
                    } else {
                        msg.innerHTML = `❌ Yanlış! Bu çeviri aslında ${gercektenDogru ? 'doğruydu' : 'yanlıştı'}.`;
                        msg.style.color = "#dc3545";
                    }
                ...
                document.getElementById('btn-dy-sonraki').onclick = () => {
                    dyIndeksi++;
                    dogruYanlisModunuCiz();
                };
            }
            ...
            actionTabs.forEach(tab => {
                tab.addEventListener('click', function() {
                    ...
                    if (mod === 'kartlar') {
                        ...
                        coktanSkor = 0;
                        coktanSecmeliModunuCiz();
                    } else if (mod === 'dogru_yanlis') {
                        dyIndeksi = 0;
                        dySkor = 0;
                        dogruYanlisModunuCiz();
                    } else if (mod === 'eslestirme') {
                        eslestirmeSayfasi = 0;
                        eslestirmeSkoru = 0;
                        eslestirmeModunuCiz();
                    }
        ...
        // =========================================================================
            ...
            const diyaloglar = Array.isArray(veri)
                ? veri
                : ((typeof egitimMufredati !== 'undefined' && egitimMufredati[secilenGun]) ? egitimMufredati[secilenGun]["diyaloglar"] : null);
            ...
            function diyalogSecimEkraniniCiz() {
                let html = `<div class="content-header"><p>${guvenliMetin(baslik)}</p></div>`;
                    ...
                    html += `
                        <div class="diyalog-kart" style="border: 1px solid #e0e0e0; border-radius: 12px; padding: 25px; background: white; box-shadow: 0 4px 6px rgba(0,0,0,0.05); display: flex; justify-content: space-between; align-items: center; transition: all 0.3s;">
                            <div style="flex: 1; padding-right: 20px;">
                                <h3 style="color: #1a5c83; margin-bottom: 10px; font-size: 22px;">💬 ${guvenliMetin(diyalog.baslik)}</h3>
                                <p style="color: #555; font-size: 15px; line-height: 1.5;">${guvenliMetin(diyalog.senaryo)}</p>
                            </div>
                ...
                document.querySelectorAll('.pratik-basla-btn').forEach(btn => {
                    btn.addEventListener('click', function() {
                        const secilenIndex = Number(this.getAttribute('data-index'));
            ...
            function diyalogPratikEkraniniCiz(diyalog) {
                ...
                let konusmaZamanlayici = null;
                const akis = Array.isArray(diyalog.akis) ? diyalog.akis : [];
                ...
                let html = `
                    <div class="content-header" style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 10px; border-bottom: 1px solid #eee;">
                        <p style="margin: 0; font-weight: 600; color: #1a5c83;">Pratik: ${guvenliMetin(diyalog.baslik)}</p>
                    ...
                    <div class="diyalog-pratik-container" style="display: flex; gap: 20px; padding: 20px; height: 600px; background: white; border-radius: 12px; margin-top: 15px; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                        <div class="sol-panel" style="flex: 1; border-radius: 12px; overflow: hidden; position: relative; background: #222; display: flex; flex-direction: column; box-shadow: inset 0 0 10px rgba(0,0,0,0.5);">
                            ...
                            <div id="etkilesim-paneli" style="min-height: 180px; background: white; padding: 25px; display: flex; flex-direction: column; justify-content: center; align-items: center;"></div>
                        </div>

                        <div class="sag-panel" style="flex: 1.2; background: #f0f2f5; border-radius: 12px; padding: 20px; overflow-y: auto; display: flex; flex-direction: column; gap: 15px; border: 1px solid #e0e0e0;" id="chat-akisi"></div>
                    </div>
                `;

                icerikAlani.innerHTML = html;

                document.getElementById('btn-diyalog-geri').addEventListener('click', function() {
                    if (konusmaZamanlayici) clearTimeout(konusmaZamanlayici);
                ...
                function siradakiMesajiOynat() {
                    if (aktifMesajIndeksi >= akis.length) {
                    ...
                    const mesaj = akis[aktifMesajIndeksi];
                    ...
                    if (mesaj.tur === 'video') {
                        ...
                        mesajDiv.innerHTML = `
                            <div style="font-size: 12px; color: #1a5c83; font-weight: 600; margin-bottom: 8px; display: flex; align-items: center; gap: 5px;">👤 ${guvenliMetin(mesaj.isim)}</div>
                            <div dir="rtl" style="font-size: 22px; margin-bottom: 8px; color: #333;">${guvenliMetin(mesaj.arapca)}</div>
                            <div style="font-size: 14px; color: #666; border-top: 1px solid #eee; padding-top: 8px;">${guvenliMetin(mesaj.turkce)}</div>
                        `;
                        ...
                        mesajDiv.innerHTML = `
                            <div style="font-size: 12px; color: #075e54; font-weight: 600; margin-bottom: 8px; display: flex; align-items: center; justify-content: flex-end; gap: 5px;">${guvenliMetin(mesaj.isim)} 👥</div>
                            <div dir="rtl" style="font-size: 22px; margin-bottom: 8px; color: #333; text-align: right;">${guvenliMetin(mesaj.arapca)}</div>
                            <div style="font-size: 14px; color: #555; text-align: right; border-top: 1px solid rgba(0,0,0,0.05); padding-top: 8px;">${guvenliMetin(mesaj.turkce)}</div>
                        `;
                        ...
                        etkilesimPaneli.innerHTML = `
                            <div style="width: 100%; display: flex; flex-direction: column; align-items: center;">
                                ...
                                <p style="color: #444; font-size: 18px; text-align: center; margin-bottom: 15px; font-weight: 600;" dir="rtl">${guvenliMetin(mesaj.arapca)}</p>
                                <button id="btn-konus" class="btn btn-primary" data-tamamlandi="false" style="padding: 14px 30px; background: #28a745; color: white; border: none; border-radius: 8px; cursor: pointer; width: 90%; display: flex; justify-content: center; align-items: center; gap: 10px; font-weight: 600; font-size: 16px; transition: all 0.3s;">
                        ...
                        document.getElementById('btn-konus').addEventListener('click', function() {
                            ...
                            if (btn.dataset.tamamlandi === "true") {
                                aktifMesajIndeksi++;
                                siradakiMesajiOynat();
                                return;
                            }
                            ...
                            konusmaZamanlayici = setTimeout(() => {
                                mesajDiv.style.display = 'block';
                                chatAkisi.scrollTop = chatAkisi.scrollHeight;

                                btn.dataset.tamamlandi = "true";
        ...
        // =========================================================================
            ...
            let htmlIcerik = `<div class="content-header"><p>${guvenliMetin(baslik)}</p></div><div class="kelime-grid">`;
            ...
            liste.forEach(kelime => {
                if (kelime.arapca && kelime.turkce) {
                    htmlIcerik += `
                        <div class="kelime-card">
                            <div class="card-top">
                                <h3 class="arapca-kelime">${guvenliMetin(kelime.arapca)}</h3>
                                <div><span style="font-size: 14px;">🔊</span></div>
                            </div>
                            <p class="turkce-anlam">${guvenliMetin(kelime.turkce)}</p>
    ...
    tumEgzersizButonlari.forEach(buton => {
        buton.addEventListener('click', function() {
            const bagliGunHeader = butonunGunHeaderiniBul(this);

            if (bagliGunHeader) {
                const bagliHaftaHeader = gununHaftaHeaderiniBul(bagliGunHeader);

                if (bagliHaftaHeader) {
                    tumHaftalariKapat(bagliHaftaHeader);
                    panelAyarla(bagliHaftaHeader, true);
                }

                tumGunleriKapat(bagliGunHeader);
                panelAyarla(bagliGunHeader, true);
            }
